...
describe('The User API', () => {
  ...
  //  curl -d "displayName=Bre444nt&uid=wMO8BJMNuydKHoNS5pVLY33Dmhi1&online=true" -X POST https://us-central1-yourvotecounts-dev.cloudfunctions.net/setUser?auth_key=eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9
   it('should be able to get and set user attributes', async () => {
    ...
    var json = await apiUser.getUser(testSupport.names[0].phoneNumber); // WE DO THIS BECAUSE WE NEED THE ACTUAL UID
    ...
    await apiUser.postUser(testSupport.names[0]);
    json = await apiUser.getUser(testSupport.names[0].phoneNumber);
    apiUser.verifyUserJson(testSupport.names[0], json, '1111111111');

    // testing just updateDisplayName()
    testSupport.names[0].displayName = origName+'YYY';
    apiUser.updateDisplayName(json['uid'], testSupport.names[0].displayName);
    json = await apiUser.getUser(testSupport.names[0].phoneNumber);
    apiUser.verifyUserJson(testSupport.names[0], json, '222222222');
    ...
    await apiUser.postUser(testSupport.names[0]);
    json = await apiUser.getUser(testSupport.names[0].phoneNumber);
    apiUser.verifyUserJson(testSupport.names[0], json, '3333333333');
  });
  ...
  it('should allow legal to be set', async () => {
    ...
    testSupport.names[0].uid = json['uid'];
    await apiUser.setLegal(testSupport.names[0], ppNew, tosNew);
    json = await apiUser.getUser(testSupport.names[0].phoneNumber);
    apiUser.verifyLegal(ppNew, tosNew, json);

    // reset
    await apiUser.setLegal(testSupport.names[0], origPp, origTos);
    json = await apiUser.getUser(testSupport.names[0].phoneNumber);
    apiUser.verifyLegal(origPp, origTos, json);
  })
